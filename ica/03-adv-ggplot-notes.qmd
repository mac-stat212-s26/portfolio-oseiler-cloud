...
- Relate the intro paragraph: "Scientists declared that 2015 was Earth’s hottest year on record..." to the *design* of the visualization.  In particular, based on the intro paragraph,
  ...
  The NYT wants people to be able to explore the difference in recorded temperatures/precipitation for 2015 and compare them to average and record temperatures/precipitation in different cities in the US. \\
  
  - How did this goal inform what information is displayed in the visualization? \\
  
  This goal informed the different layers on the plot, as well as the severity of the colors for the different types of data on the plot. \\
...
Every variable in the codebook seems to be represented in the plot in some way.


- How do these variables map to aesthetics of the visual elements, eg, position, size, shape, and color of glyphs? \\

The variables recording temperature are the histograms in the plot, temperature and Month are the Axes, and the record high/lows have arrows pointing to them on the plot. This is similar in the precipitation plot, however  
...
P <- ggplot(weather, aes(x = dateInYear)) + 
...
P <- ggplot(weather, aes(x = dateInYear)) + 
  ...
  geom_linerange(aes(ymin = Low, ymax = High), color = '#A90248', linewidth = 1.05) +
  theme_classic()
...
I think we can use either geom_abline or geom_hline here. We could maybe use the 'Day' variable and put the line at the first day for each month? We could also use brute force and use DayInYear and identify the days which are the end of months and pass in 12 different lines.  
...
P <- ggplot(weather, aes(x = dateInYear)) + 
  ...
  geom_linerange(aes(ymin = NormalLow, ymax = NormalHigh), color = '#C8B8BA') +
  geom_linerange(aes(ymin = Low, ymax = High), color = '#A90248', linewidth = 1.05) +
  geom_vline(data = Month_lines, aes(xintercept = dateInYear ), alpha = 0.3) +
  theme_classic()
...
Month_center <- weather %>%
  group_by(Month) %>%
  summarize(med = median(dateInYear))
...
P <- ggplot(weather, aes(x = dateInYear)) + 
  ...
  geom_linerange(aes(ymin = Low, ymax = High), color = '#A90248', linewidth = 1.05) +
  geom_vline(data = Month_lines, aes(xintercept = dateInYear ), alpha = 0.3) +
  scale_x_continuous(breaks = Month_center$med, labels = month.abb) +
  theme_classic()
