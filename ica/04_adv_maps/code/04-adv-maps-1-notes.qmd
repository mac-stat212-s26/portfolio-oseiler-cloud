...
transform_and_plot(nc, new_crs = "EPSG:27291", title = "North Carolina mapped with NZ CRS")

transform_and_plot(nc, new_crs = "EPSG:29702", title = "North Carolina mapped with Madagascar CRS")
...
any overlay between the location data under different CRS's would not line up accurately.
...
st_crs(mn_cities)
st_crs(mn_water)
st_crs(mn_counties) #needs to change to NAD83

```

```{r}
st_transform(mn_counties, crs = st_crs(mn_cities), title = "none")
```
...
ggplot() +
    ...
    theme_classic() +
    ggthemes::theme_map() +
    labs(title = "Minnesota counties and Cities")
...
- Read the function description at the top. What is the advantage of using this function for making color palettes?
  This function is uniform both when in color, and when in black-and white. They are designed to be visible to those with color blindness as well. 

- Look through the examples section. What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function?
  '_d()' is used for discrete data, '_c()' is used for continuous data, and '_b()' is used to bin continuous data before mapping.
...
```{r}
ggplot() +
    geom_raster(data = elev_df, aes(x = x, y = y, fill = elevation)) + # elevation as first layer
...
png("../figures/tc_map_zoom.png", width = 800, height = 500)
ggplot() +
    geom_raster(data = elev_df, aes(x = x, y = y, fill = elevation)) + 
    geom_sf(data = mn_counties, fill = NA, color = "black") + # county boundary layer
    geom_sf(data = mn_water, fill = "lightsteelblue1", color = "lightsteelblue1") + # NEW: river/lake layer
