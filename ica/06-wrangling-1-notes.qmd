...
#2
diamonds %>%
  filter(500 <= price, price <= 600)
#3
# should get error of longer object not multiple of shorter length
diamonds %>%
    mutate(fpi = cut == c("Fair", "Premium", "Ideal")) %>% 
    summarize(num_fpi = sum(fpi), frac_fpi = mean(fpi))
## Right way 
diamonds %>%
    mutate(fpi = cut %in% c("Fair", "Premium", "Ideal")) %>% 
    summarize(num_fpi = sum(fpi), frac_fpi = mean(fpi))
#4
diamonds %>%
  filter(cut == 'fair') %>%
  summarize(f03 = any(price > 3000))

diamonds %>%
  filter(cut == 'ideal') %>%
  summarize(i02 = all(price > 2000))

#5
diamonds %>%
  mutate(price_cat1 = if_else(price < 500, 'low','high'), price_cat2 = case_when(price < 500 ~'low', price >= 500 & price <= 1000 ~'medium', price > 1000 ~'high'))
...
```{r}
new_ratings <- fct(ratings, levels = c("Low", "Medium", "High"))
new_ratings
```
