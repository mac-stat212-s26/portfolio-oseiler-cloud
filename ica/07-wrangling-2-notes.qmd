...
```{r ex_create_string}
# Your code
# Doesn't work because of the apostrophe my_string <- 'It's Thursday
my_string <- "It's Thursday"
```
...
df_dates %>%
  mutate(mdy1 = str_c(month, "-", day, "-", year),
         mdy2 = str_glue("{month}-{day}-{year}"))
...
df %>%
  mutate(len = str_length(word), 
         ...
         middleletter = str_sub(word, start = middle, end = middle))
...
# Your code
str_view(fruit, "[a][a]|[e][e]|[i][i]|[o][o]|[u][u]")
