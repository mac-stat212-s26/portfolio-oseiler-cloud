...
vis_miss(airquality) # Where are NA's located?
miss_var_summary(airquality) # Information from vis_miss() in table form
```
...
ggplot(airquality_mod, aes(x = Solar.R_mod2, y = Ozone)) +
    ...
    labs(title = "Alternate Situation 2")
```
In both situations, the ozone is na for high solar values.
It will be easier to 'deal' with the missing data for situation 1, as there is other data points to extrapolate and impute the missing values from, whereas in situation 2, the missing values are outside of the range of the solar data. 
...
```{r}
# How does month relate to na values
ggplot(airquality, aes(x = factor(Month), y = Ozone)) +
    geom_boxplot()

# how is wind related to na values
ggplot(airquality, aes(x = is.na(Ozone), y = Wind)) +
    geom_boxplot()

#how is temp related to na values?
ggplot(airquality, aes(x = is.na(Ozone), y = Temp)) +
    geom_boxplot()
```

Seems to be related to month; june or august
Can we user linear regression? Not for month but maybe from temp or wind
...
Yes! While we may not be able to impute all the misisng values, we can extrapolate most of them. 
