...
```{r}
formatphonenum <- function(n){
  nums <- str_remove_all(n, "[^[:alnum:]]")
...
formatphonenum(c("651-330-8661", "6516966000", "800 867 5309"))
...
```
for geom_sf, mapping goes first, then data. The error is due to the fact that these are switched.
...
```{r}
domainName <- function(s){
  str_extract(s, "@.+$") %>%
    str_remove("@")
}

domainName("oseiler@macalester.edu")
...
prop <- function(df, group_var, mean_var) {
  df %>%
    group_by(pick({{ group_var }})) %>%
    summarize(mean = mean({{ mean_var }})) %>%
    props = mean/(length(df))
    return(props)
}
prop(diamonds, group_var = cut, mean_var = carat)
...
scattersmooth(diamonds, x = carat, y = price)
```
## Done!
