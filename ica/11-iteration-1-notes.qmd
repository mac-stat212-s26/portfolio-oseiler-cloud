...
```{r}    
# for loop
month_str <- vector("character", length = length(month.name)) #storage vector

for (i in seq_along(month.name)) {
    month_str[i] <- str_c("Month ", i, ": ", month.name[i])
    return(month_str)
}
...
lin_mod_results <- vector(mode = "list", length = length(PvC_models)) #storage container

# for loop

for (i in seq_along(PvC_models)) {
...
diamonds %>%
  mutate(across(.cols = c(x,y,z), .fns = mms))
