...
```{r}
mt_classes <- vector("character", ncol(mtcars)) #create storage container

# loop over the columns; each column class is a character in the vector defined above
for (i in seq_along(mtcars)) {
    mt_classes[i] <- class(mtcars[[i]])
}
```
...
```{r}
map_df(diamonds %>% select(where(is.numeric)), summ_stats)
```
